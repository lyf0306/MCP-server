...
class ToolUniverse:
    def __init__(self, tool_files=default_tool_files, keep_default_tools=True):
        ...
        logging.debug(f"tool_files={tool_files}")
        self.callable_functions = {}

    def load_tools(self, tool_type=None):
        logging.debug(f"Number of tools before load tools: {len(self.all_tools)}")
        ...
        logging.info(f"Number of tools after load tools: {len(self.all_tools)}")
    ...
    def refresh_tool_name_desc(self, enable_full_desc=False):
        print(f"enable_full_desc: {enable_full_desc}")
        tool_name_list = []
        tool_desc_list = []
