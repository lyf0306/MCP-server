...
def search_openfda(params=None, endpoint_url=None, api_key=None, sort=None, limit=5, 
    ...
    try:
        ...
        print(f"Request failed or failed to decode JSON: {e}")
        # 如果发生异常，返回空结构，防止后续代码报错
        return {
            'meta': {},
            'results': []
        }

    # [修复 1] 处理 API 直接返回列表的情况
    ...
    # [关键修复 4] 极其鲁棒的 meta 信息提取逻辑
    # 原因：response_data.get('meta') 可能返回列表 []，导致后续调用 .get() 报错
    raw_meta = response_data.get('meta', {})
    meta_info = {}
    
    if isinstance(raw_meta, dict):
        # 只有当 meta 是字典时，才尝试获取内部的 results
        meta_results = raw_meta.get('results')
        if isinstance(meta_results, dict):
            meta_info = meta_results
    # 如果 meta 是列表或其他类型，meta_info 保持为 {}，避免报错
    ...
    required_fields = list(search_fields.keys()) + (return_fields if return_fields else [])
